...
__all__ = ["RAdamAccumulator"]


class RAdamAccumulator(keras.optimizers.Optimizer):
    """RAdamAccumulator optimizer.
    ...
    def __init__(
        ...
        **kwargs
        ...
        super(RAdamAccumulator, self).__init__(**kwargs)
        with K.name_scope(self.__class__.__name__):
            ...
            self.warmup_proportion = K.variable(
                warmup_proportion, name="warmup_proportion"
            )
            self.min_lr = K.variable(lr, name="min_lr")
            self.accum_iters = K.variable(lr, name="accum_iters")
        if epsilon is None:
            epsilon = K.epsilon()
    ...
    def get_updates(self, loss, params):
        ...
        gs = [
            K.zeros(K.int_shape(p), dtype=K.dtype(p), name="g_" + str(i))
            for (i, p) in enumerate(params)
        ]
        ...
        for p, g, m, v, vhat, tg in zip(params, grads, ms, vs, vhats, gs):
            ...
            v_t = (self.beta_2 * v) + (1.0 - self.beta_2) * K.square(avg_grad)
    ...
    def get_config(self):
        ...
        base_config = super(RAdamAccumulator, self).get_config()
